...
case $ARCHITECTURE in
  arm|arm64)
    ...
    ;;
  *)
    [ -e $DOWNLOADS/ks.tar.gz ] || curl -sLf --retry 3 -o $DOWNLOADS/ks.tar.gz https://github.com/ksonnet/ksonnet/releases/download/v$KSONNET_VERSION/ks_$KSONNET_VERSION_linux_$ARCHITECTURE.tar.gz
    tar -C /tmp -xf $DOWNLOADS/ks.tar.gz
    cp /tmp/ks_$KSONNET_VERSION_linux_$ARCHITECTURE/ks $BIN/ks
    ;;
esac
